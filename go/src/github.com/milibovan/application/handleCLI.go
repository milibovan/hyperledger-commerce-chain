...
func handleCLI() {
	...
	case "create-trader":
		...
		email := createTraderCmd.String("email", "some@gmail.com", "email")
		...
		executeCLI(*org, *user, *channel, func() {
			handleCLICreateTrader(*channel, *name, *traderType, *vat, *email, *balance)
		})
	...
	case "add-product-to-trader":
		...
		args := addProductCmd.String("args", "", "Arguments (e.g., 'trader-id,product-a,10,product-b,20')")

		addProductCmd.Parse(os.Args[2:])
		executeCLI(*org, *user, *channel, func() {
			handleCLIAddProductToTrader(*channel, *args)
		})
...
func handleCLICreateTrader(channelName, name, traderType, vat, email, balance string) {
	blockNumber, ID := client.CreateTrader(activeGW, channelName, name, traderType, vat, email, balance)
...
func handleCLIAddProductToTrader(channelName string, args string) {
	//blockNumber, _ := client.AddProductsToTrader(activeGW, channelName, args)

	printInfo(fmt.Sprintf("\nProduct was added successfully on channel %s.", channelName))
