...
function checkPrereqs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    CA_DOCKER_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm hyperledger/fabric-ca:2.5 fabric-ca-client version | sed -ne 's/ Version: //p' | head -1)
...
#  COMPOSE_FILE_BASE=$temp_compose
  ...
  if [ "$MODE" != "restart" ]; then
    infoln "Removing leftover volumes and artifacts"
...
#    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.org1.example.com docker_peer0.org2.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/org3/msp organizations/fabric-ca/org3/tls-cert.pem organizations/fabric-ca/org3/ca-cert.pem organizations/fabric-ca/org3/fabric-ca-server.db'
