...
# Set TLS and Admin credentials for the raft0 Orderer
export ORDERER_ADMIN_TLS_ROOTCERT_FILE=${PWD}/organizations/ordererOrganizations/example.com/orderers/raft0.example.com/tls/ca.crt
...
infoln "Orderer raft0.example.com joining channel ${CHANNEL_NAME}"

# Join the application channel using the Orderer Admin API (osnadmin)
# The orderer node's peer address is raft0.example.com:7050 (from configtx/docker-compose)
osnadmin channel join --channelID $CHANNEL_NAME \
  --config-block ./channel-artifacts/${CHANNEL_NAME}.block \
  --ordererTLSHostnameOverride raft0.example.com \
