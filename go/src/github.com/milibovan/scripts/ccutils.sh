...
function installChaincode() {
  ...
  if test $? -ne 0; then
    peer lifecycle chaincode install packagedChaincode/${CC_NAME}.tar.gz
    res=$?
  fi
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvoke() {
  ...
  res=$?
  verifyResult $res "Invoke transaction failed due to error in peer parameters"
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    ...
    set -x
    peer chaincode invoke -o localhost:7050 -C $CHANNEL_NAME -n ${CC_NAME} -c ${CC_INVOKE_CONSTRUCTOR} --tls --cafile $ORDERER_CA "${PEER_CONN_PARMS[@]}" >&log.txt
    res=$?
    { set +x; } 2>/dev/null
    let rc=$res
    COUNTER=$(expr $COUNTER + 1)
