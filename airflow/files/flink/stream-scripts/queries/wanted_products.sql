...
BEGIN
  ...
  WITH sales_hop AS (
    SELECT
      ...
      HOP_START(rc.event_time, INTERVAL '30' SECOND, INTERVAL '1' MINUTE) AS w_start,
      HOP_END(rc.event_time,   INTERVAL '30' SECOND, INTERVAL '1' MINUTE) AS w_end
    FROM receipt_created rc
    JOIN receipt_created_products rcp
      ON  rcp.event_id   = rc.event_id
      AND rcp.event_time BETWEEN rc.event_time - INTERVAL '1' SECOND
                              AND rc.event_time + INTERVAL '1' SECOND
    GROUP BY
      rcp.product_id,
      HOP(rc.event_time, INTERVAL '30' SECOND, INTERVAL '1' MINUTE)
  ),
  ...
  demand_hop AS (
    ...
    JOIN request_created_products rcp
      ON  rcp.event_id   = rc.event_id
      AND rcp.event_time BETWEEN rc.event_time - INTERVAL '1' SECOND
                              AND rc.event_time + INTERVAL '1' SECOND
    GROUP BY
  ...
  sales_growth AS (
    SELECT
      ...
      ROUND(
        s_curr.total_sales
        / NULLIF(CAST(GREATEST(s_curr.unique_users, 1) AS DOUBLE), 0)
      , 4) AS sales_coeff
    FROM sales_hop s_curr
    JOIN sales_hop s_prev
      ON  s_curr.product_id = s_prev.product_id
      AND s_curr.w_start    = s_prev.w_start + INTERVAL '30' SECOND
  ),
  ...
  demand_growth AS (
    SELECT
      ...
      ROUND(
        d_curr.total_requests
        / NULLIF(CAST(GREATEST(d_curr.unique_users, 1) AS DOUBLE), 0)
      , 4) AS demand_coeff
    FROM demand_hop d_curr
    JOIN demand_hop d_prev
      ON  d_curr.product_id = d_prev.product_id
      AND d_curr.w_start    = d_prev.w_start + INTERVAL '30' SECOND
  ),
  ...
  combined AS (
    ...
    FULL OUTER JOIN demand_growth dg
      -- removed w_start from join condition to avoid window timing mismatch
