from pyflink.table import EnvironmentSettings, TableEnvironment
import os

def run_transformation():
    settings = EnvironmentSettings.new_instance().in_batch_mode().build()
    t_env = TableEnvironment.create(settings)

    lib_dir = "/opt/flink/lib/"
    if os.path.exists(lib_dir):
        jars = [f"file://{os.path.join(lib_dir, jar)}" for jar in os.listdir(lib_dir) if jar.endswith(".jar")]
        jar_string = ";".join(jars)
        t_env.get_config().set("pipeline.jars", jar_string)
        t_env.get_config().set("pipeline.classpaths", jar_string)

    t_env.execute_sql("""
        CREATE TABLE transform_receipts (
            `doc-type` STRING,
            id STRING,
            `trader-id` STRING,
            `user-id` STRING,
            `order-id` STRING,
            products ARRAY<ROW<`product_id` STRING, quantity INT>>,
            `date` TIMESTAMP(3),
            `total-cost` DOUBLE,
            `calculated-cost` DOUBLE,
            `cost-variance` DOUBLE,
            status STRING,
            deleted BOOLEAN,
            product_count BIGINT NOT NULL,
            total_items INT,
            order_status STRING
        ) WITH (
            'connector' = 'filesystem',
            'path' = 'hdfs://namenode:9000/datalake/transform/receipts_transformed.parquet',
            'format' = 'parquet'
        )
    """)

    t_env.execute_sql("""
        CREATE TABLE transform_traders (
            id STRING,
            name STRING,
            `trader-type` STRING,
            `receipts-ids` ARRAY<STRING>,
            deleted BOOLEAN
        ) WITH (
            'connector' = 'filesystem',
            'path' = 'hdfs://namenode:9000/datalake/transform/traders_transformed.parquet',
            'format' = 'parquet'
        )
    """)

    t_env.execute_sql("""
        CREATE TABLE transform_orders (
            `doc-type` STRING,
            id STRING,
            `user-id` STRING,
            status STRING,
            `created-date` TIMESTAMP(3),
            products ARRAY<ROW<`product_id` STRING, quantity INT>>,
            `receipts-ids` ARRAY<STRING>,
            `total-cost` DOUBLE,
            `calculated-cost` DOUBLE,
            `cost-variance` DOUBLE,
            `request-id` STRING,
            deleted BOOLEAN,
            product_count BIGINT NOT NULL,
            total_items INT
        ) WITH (
            'connector' = 'filesystem',
            'path' = 'hdfs://namenode:9000/datalake/transform/orders_transformed.parquet',
            'format' = 'parquet'
        )
    """)

    t_env.execute_sql("""
        CREATE TABLE rank_traders (
            trader_id STRING,
            trader_name STRING,
            trader_type STRING,
            receipts_count BIGINT,
            orders_count BIGINT,
            total_cost DOUBLE,
            product_count BIGINT,
            avg_realization_speed_days DOUBLE
        ) WITH (
            'connector' = 'jdbc',
            'url' = 'jdbc:postgresql://citus_coordinator:5432/curated_zone',
            'table-name' = 'rank_traders',
            'username' = 'postgres',
            'password' = '0Hf9Vnnxe3Cay5ZE',
            'driver' = 'org.postgresql.Driver',
            'sink.buffer-flush.max-rows' = '100',
            'sink.buffer-flush.interval' = '1s',
            'sink.max-retries' = '3'
        )
    """)

    # INSERT: izračun prosječnog vremena reakcije od created-date narudžbe do datuma računa (u danima)
    t_env.execute_sql("""
        INSERT INTO rank_traders
        SELECT
            tt.id AS trader_id,
            tt.name AS trader_name,
            tt.`trader-type` AS trader_type,
            COUNT(tr.id) AS receipts_count,
            COUNT(DISTINCT o.id) AS orders_count,
            SUM(o.`total-cost`) AS total_cost,
            SUM(o.product_count) AS product_count,
            AVG(
                TIMESTAMPDIFF(HOUR,
                    o.`created-date`,
                    tr.`date`
                )
            ) / 24.0 AS avg_realization_speed_days
        FROM transform_traders tt
        JOIN transform_receipts tr
            ON tt.id = tr.`trader-id`
        JOIN transform_orders o
            ON tr.`order-id` = o.id
        WHERE tt.deleted = FALSE
        AND tr.deleted = FALSE
        AND (o.deleted IS NULL OR o.deleted = FALSE)
        GROUP BY tt.id, tt.name, tt.`trader-type`
    """).wait()

    print("✓ Data transformation complete: results written to Citus.")

if __name__ == '__main__':
    run_transformation()
