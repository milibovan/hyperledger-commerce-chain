...
with DAG(
    dag_id='unsuccessful_requests_value',
    ...
    create_table = SQLExecuteQueryOperator(
        ...
        conn_id='citus',
        sql="""
            DROP TABLE IF EXISTS unsuccessful_requests_value;
            
            CREATE TABLE IF NOT EXISTS unsuccessful_requests_value (
                status VARCHAR(16),
                trader_type VARCHAR(66),
                user_tier VARCHAR(5),
                avg_value DOUBLE PRECISION,
                request_count INTEGER,
                avg_product_count DOUBLE PRECISION,
                avg_validity_days DOUBLE PRECISION,
                PRIMARY KEY (status, trader_type, user_tier)
            );
        """,
    )
    ...
    unsuccessful_requests_value = PythonOperator(
        task_id='flink_unsuccessful_requests_value',
        python_callable=submit_pyflink_job,
        op_kwargs={'script_name': 'unsuccessful_requests_value.py'}
    )
    
    verify_citus_data = SQLExecuteQueryOperator(
        task_id='verify_citus_results',
        conn_id='citus',
        sql="SELECT COUNT(*) FROM unsuccessful_requests_value;",
    )
    
    hdfs_input_path = check_hdfs_path("/datalake/transform/")
    
    check_citus >> hdfs_input_path >> create_table >> unsuccessful_requests_value >> verify_citus_data