...
with DAG(
    dag_id='inventory_value',
    start_date=datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    tags=['flink', 'citus', 'hdfs', '09']
) as dag:
    ...
    create_table = SQLExecuteQueryOperator(
        ...
        sql="""
            CREATE TABLE IF NOT EXISTS inventory_value (
                trader_name VARCHAR(56),
                trader_type VARCHAR(56),
    ...
    avg_calc = PythonOperator(
        ...
        op_kwargs={'script_name': 'inventory_value.py'}
    )
    
    verify_citus_data = SQLExecuteQueryOperator(
        task_id='verify_citus_results',
        conn_id='citus',
        sql="SELECT COUNT(*) FROM inventory_value;",
    )
    ...
    truncate_table = SQLExecuteQueryOperator(
        ...
        conn_id='citus',
        sql="TRUNCATE TABLE inventory_value;",
    )
